...
const FocusTimer: React.FC = () => {
  ...
  const handleSaveNote = () => {
    if (dailyNote.trim()) {
      submitDailyNote(dailyNote); // +10 points
      setNoteSaved(true);
  ...
  const handleAddFlashcard = (e: React.FormEvent) => {
    e.preventDefault();
    if (newCardText.trim()) {
      addFlashCard(newCardText, newCardInterval);
      setNewCardText("");
          ...
          <div className="bg-white rounded-2xl shadow-md p-6">
            ...
            <div className="flex justify-end mt-3">
              <button 
                ...
                className="flex items-center bg-indigo-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-indigo-700 transition-all"
              >
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-3 mb-6">
              {dailyGoals?.map(goal => (
                <div key={goal.id} className="flex items-start space-x-3 group cursor-pointer" onClick={() => toggleGoal(goal.id)}>
                  ...
                  <p className={`text-sm transition-all ${goal.completed ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                    {goal.text}
                  </p>
                </div>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
            ...
            <div className="space-y-4 mb-6 max-h-48 overflow-y-auto">
              ...
              {(!flashCards || flashCards.length === 0) && <p className="text-sm text-gray-400 italic">No reminders set.</p>}
            </div>
            ...
            <form onSubmit={handleAddFlashcard} className="space-y-3 border-t border-gray-100 pt-4">
              ...
              <div className="flex gap-2">
                <select 
                  ...
                  onChange={(e) => setNewCardInterval(e.target.value as 'hourly' | 'daily')}
                >
                  <option value="hourly">Hourly</option>
                  <option value="daily">Daily</option>
                </select>
                <button type="submit" className="bg-yellow-500 text-white px-4 py-2 rounded text-sm font-bold">
                  Set
                </button>
              </div>
            </form>
