...
const ReaderModal: React.FC<ReaderModalProps> = ({ item, onClose }) => {
  ...
  // Notes (local state for smooth typing)
    ...
    <div className="fixed inset-0 bg-white z-50 flex flex-col animate-fade-in">
      ...
      <div className="bg-white border-b border-gray-200 p-4 px-8 flex justify-between items-center shadow-sm z-50">
        <div className="flex items-center overflow-hidden">
          <button
            onClick={handleClose}
            className="mr-4 p-2 hover:bg-gray-100 rounded-full transition-colors text-gray-500 flex-shrink-0"
          >
            <X size={24} />
          </button>
        ...
        <div className="flex items-center space-x-6 bg-gray-50 px-6 py-2 rounded-xl border border-gray-200 flex-shrink-0">
          <div
            className={`text-2xl font-mono font-bold ${
              isReading ? 'text-indigo-600' : 'text-gray-400'
            }`}
          >
          ...
          {readingSeconds > 0 && (
            <button
              onClick={handleClose}
              className="text-red-500 font-semibold text-sm hover:underline hidden md:block"
            >
              Stop & Save
            </button>
          )}
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-y-auto">
        {/* CONTENT AREA */}
        <div className="flex-1 bg-gray-50 p-0 lg:p-6 flex flex-col">
          {/* ✅ FIXED: removed overflow-hidden */}
          <div className="w-full h-full bg-white shadow-sm rounded-none lg:rounded-xl relative">
            {item.type === 'link' ? (
              <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                ...
                <p className="text-gray-500 mb-8 max-w-md">
                  This material is hosted externally. Keep this timer running
              ...
              <iframe
                src={item.content}
                className="w-full h-[calc(100vh-120px)] lg:h-full border-none"
                title="PDF Viewer"
              />
            ) : (
              <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                ...
                >
                  Download File
                  <Upload size={18} className="ml-2 transform rotate-180" />
                </a>
              </div>
            )}
          </div>
