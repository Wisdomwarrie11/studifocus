...
import { CheckCircle, Circle, Flame, Trophy, ArrowRight, Plus, Trash2, Edit2 } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';

const StudentDashboard: React.FC = () => {
  const { user, dailyGoals, toggleGoal, addGoal, editGoal, deleteGoal, weeks, announcements } = useApp();
                  ...
                  <div className="flex items-center justify-between group py-1.5 px-2 rounded-xl hover:bg-gray-50/80 border border-transparent transition-all">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className={`text-sm md:text-base break-words transition-all ${goal.completed ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                          {goal.text}
                        </p>
                        {goal.createdAt && (
